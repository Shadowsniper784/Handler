module.exports = {
    ...
    execute(client, message) {
        ...
        if (!client.player.getQueue(message)) return message.send(`${client.emotes.error} - No songs currently playing !`);

        message.send(`**Server queue - ${message.guild.name} ${client.emotes.queue} ${client.player.getQueue(message).loopMode ? '(looped)' : ''}**\nCurrent : ${queue.playing.title} | ${queue.playing.author}\n\n` + (queue.tracks.map((track, i) => {
