module.exports = {
    ...
    execute(client, message) {
        ...
        if (client.player.getQueue(message).tracks.length <= 1) return message.send(`${client.emotes.error} - There is only one song in the queue.`);

        client.player.clearQueue(message);

        message.send(`${client.emotes.success} - The queue has just been **removed** !`);
    },
};