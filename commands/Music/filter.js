...
module.exports = {
    ...
    execute(client, message, args) {
        ...
        if (!filterToUpdate) return message.send(`${client.emotes.error} - This filter doesn't exist, try for example (8D, vibrato, pulsator...) !`);
        ...
        if (filtersUpdated[filterToUpdate]) message.send(`${client.emotes.music} - I'm **adding** the filter to the music, please wait... Note : the longer the music is, the longer this will take.`);
        else message.send(`${client.emotes.music} - I'm **disabling** the filter on the music, please wait... Note : the longer the music is playing, the longer this will take.`);
    },
};