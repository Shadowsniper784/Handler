...
module.exports = {
    ...
    execute(client, message, args) {
        ...
        if (!args[0] || isNaN(args[0]) || args[0] === 'Infinity') return message.send(`${client.emotes.error} - Please enter a valid number !`);

        if (Math.round(parseInt(args[0])) < 1 || Math.round(parseInt(args[0])) > 100) return message.send(`${client.emotes.error} - Please enter a valid number (between 1 and 100) !`);

        const success = client.player.setVolume(message, parseInt(args[0]));

        if (success) message.send(`${client.emotes.success} - Volume set to **${parseInt(args[0])}%** !`);
    },
};