module.exports = {
    ...
    execute(client, message) {
        ...
        if (!client.player.getQueue(message).paused) return message.send(`${client.emotes.error} - The music is already playing !`);

        const success = client.player.resume(message);

        if (success) message.send(`${client.emotes.success} - Song ${client.player.getQueue(message).playing.title} resumed !`);
    },
};