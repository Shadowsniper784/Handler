    ...
    execute(client, message, args) {
        ...
        if (args.join(" ").toLowerCase() === 'queue') {
            if (client.player.getQueue(message).loopMode) {
                ...
                return message.send(`${client.emotes.success} - Repeat mode **disabled** !`);
            } else {
                client.player.setLoopMode(message, true);
                return message.send(`${client.emotes.success} - Repeat mode **enabled** the whole queue will be repeated endlessly !`);
            };
        } else {
            if (client.player.getQueue(message).repeatMode) {
                ...
                return message.send(`${client.emotes.success} - Repeat mode **enabled** the current music will be repeated endlessly !`);
            };
        };
    },
};