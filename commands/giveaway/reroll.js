module.exports = {
  ...
  callback: ({ message, args, client }) => {
    ...
    if(parseInt(args[0])) giveaway = client.giveawaysManager.giveaways.find((g) => g.messageID === args[0])
     // Search with giveaway prize
    else if(args[0]) giveaway = client.giveawaysManager.giveaways.find((g) => g.prize === args.join(' '))
    ...
    .then(() => {
        // Success message
        message.send('Giveaway rerolled!');
    })
    .catch((e) => {
        if(e.startsWith(`Giveaway with message ID ${giveaway.messageID} is not ended.`)){
            message.send('This giveaway is not ended!');
        } else {
            console.error(e);
